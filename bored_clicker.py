...
def getauto0():
    ...
    clickcount['text'] = countvar.get()
    print(f'[{datetime.now().strftime("%H:%M:%S")}] Autoclicker (BASIC): Accumulated {accumulated}, click count is now {countvar.get()}.')
...
def getauto1():
    ...
    clickcount['text'] = countvar.get()
    print(f'[{datetime.now().strftime("%H:%M:%S")}] Autoclicker (PREMIUM): Accumulated {accumulated}, click count is now {countvar.get()}.')
...
def upgradefunc0():
    ...
    if upgrade0var == 20 and not override:
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade: Clicks level has reached the maximum Level{upgrade0var}. Cannot upgrade item unless overriden.')
        return
    ...
    if countvar.get() < cost0:
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade: Insufficient clicks for upgrading clicks level.')
        return
    ...
    if upgrade0var == 20 and not override:
        upgrade0['text'] = f'Clickss - Level{upgrade0var} (MAXED)'
        button0.config(state='disable', text='MAXED')
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade: Clicks from Level{upgrade0var - 1} to Level{upgrade0var} (MAXED). The amount of increment is now {perclick}.')
    elif upgrade0var <= 20 and not override:
        upgrade0['text'] = f'Clickss - Level{upgrade0var}'
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade: Clicks from Level{upgrade0var - 1} to Level{upgrade0var}. The amount of increment is now {perclick}.')
    elif override:
        upgrade0['text'] = f'Clickss - Level{upgrade0var}'
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade (OVERRIDEN): Clicks from Level{upgrade0var - 1} to Level{upgrade0var}. The amount of increment is now {perclick}.')
...
def upgradefunc1():
    ...
    if upgrade1var == 20 and not override:
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade: Autoclicker has reached the maximum Level{upgrade1var}. Cannot upgrade item unless overriden.')
        return
    ...
    if countvar.get() < cost1:
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade: Insufficient clicks for upgrading Autoclicker level.')
        return
    # display next level's cost
    ...
    if upgrade1var == 20 and not override:
        upgrade1['text'] = f'Clicks - Level{upgrade1var} (MAXED)'
        button1.config(state='disable', text='MAXED')
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade: Autoclicks from Level{upgrade1var - 1} to Level{upgrade1var} (MAXED). The amount of increment is now {perauto0}.')
    elif upgrade1var <= 20 and not override:
        upgrade1['text'] = f'Clicks - Level{upgrade1var}'
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade: Autoclicks from Level{upgrade1var - 1} to Level{upgrade1var}. The amount of increment is now {perauto0}.')
    elif override:
        upgrade1['text'] = f'Clicks - Level{upgrade1var}'
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Upgrade (OVERRIDEN): Autoclicks from Level{upgrade1var - 1} to Level{upgrade1var}. The amount of increment is now {perauto0}.')
...
def cheats():
    ...
    countvar.set(countvar.get() + 999999999999)
...
def banner():
    ...
    while True:
        ...
        if countvar.get() == 299792458 and not got299792458:
            ...
            got299792458 = True
            print(f'[{datetime.now().strftime("%H:%M:%S")}] Found easter egg: In the speed of light')
...
win.title('bored clicker v0.1')
win.config(bg='#000000')
win.wm_attributes('-alpha', '0.9')

# --------global variables
global perclick, perauto0, cost0, cost1, override, upgrade0var, menuopened
menuopened = False
perclick = 1
perauto0 = 1
perauto1 = 2
...
upgrade1 = Label(win, text='Autoclicker - Level1', font=('Arial', 12), fg='#ffffff', bg='#000000')
upgrade1.pack()
